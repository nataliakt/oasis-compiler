...
class OasisLexicalTest {
    ...
    @BeforeEach
    void setUp() {
        lexicalAnalyzer = new OasisLexical();
    }
    ...
    void testNewClass() {
        ...
        assertEquals("<MID,Classe> ", tokens.get(count++).toString(), "Id de classe não idetificado");
        assertEquals("<{,{> ", tokens.get(count++).toString(), "Id de abertura de chaves não idetificado");
        assertEquals("<},}> ", tokens.get(count++).toString(), "Id de fechamento de chaves não idetificado");
    }

    @Test
    void testTypes() {
        List<Token> tokens = lexicalAnalyzer.analyze("integer decimal bit string " +
                "'olá' 1 2.2 true false");

        int count = 0;
        assertEquals(9, tokens.size(), "Quantidade de tokens inesperada");
